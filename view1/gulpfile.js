...
var file_road = {
    w_cleanall:['../bin/static/{css,fonts,img,js}','../bin/template/index.html'],//删除文件——静态文件（css fonts img js）和html（index_in.html）

    static_src:'./dist/{css,fonts,img,js}/**/*',//待复制静态文件
    html_src:'./dist/index.html',//待复制html
    static_dst:'../bin/static',//静态文件-online目录
    html_dst:'../bin/template',//html-online目录
};

// 清空bin目录下的静态文件（css fonts img js）和html（index_in.html）------------------------------------------------------------------------------------------------------------------------------------------
gulp.task('del', function(cb) {//不删除图片
    del(file_road.w_cleanall, { force: true }, cb);
});
//文件转移------------------------------------------------------------------------------------------------------------------------------------------
gulp.task('copy_static', function() {//复制静态文件到bin/static
    gulp.src(file_road.static_src)
        .pipe(gulp.dest(file_road.static_dst))
});
gulp.task('copy_html', function() {//复制到bin/template
    gulp.src(file_road.html_src)
