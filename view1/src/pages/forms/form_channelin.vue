<template>
    <div>
        <div class="right_head">
            {{$t('chanelIn.titleBank.title')}}
        </div>
        <div class="right_body forms_form">
            <div class="line_pic">
                <div class="select wid_long">
                    <span class="sel_title">{{$t('chanelIn.bodyBank.selTime')}}：</span>
                    <el-date-picker v-model="sel_day" type="date" :placeholder="$t('chanelIn.bodyBank.timePlace')" :picker-options="pickerOptions" @change="change_date" format="yyyy-MM-dd">
                    </el-date-picker>
                </div>
                <div class="line_body" id="form_pic"></div>
                <h3 class="note">{{$t('chanelIn.bodyBank.desDate')}}：<span class="orange">{{note_data.start_date}}</span>-<span class="orange">{{note_data.end_date}}</span> ，{{$t('chanelIn.bodyBank.desChannel')}}：<span class="orange">{{note_data.mchnt_nums}}</span>{{$t('chanelIn.bodyBank.unitPerson')}}，{{$t('chanelIn.bodyBank.desChannelNum')}}：<span class="orange">{{note_data.sub_qd_cnt}}</span>{{$t('chanelIn.bodyBank.unitGe')}}。</h3>
            </div>
        </div>
    ...
    methods: {
        ...
        formpic_line: function() {
            ...
            _this.from_now.setOption({
                ...
                xAxis: {
                    name: _this.$t('chanelIn.bodyBank.lineX'),
                    data: _this.form_x_data.slice(0, _this.data_len),
                },
                yAxis: {
                    name: _this.$t('chanelIn.bodyBank.lineY'),
                    type: 'value',
                    boundaryGap: ['0%', '5%'],
                },
                series: [{
