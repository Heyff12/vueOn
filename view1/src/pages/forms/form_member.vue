<template>
    <div>
        <div class="right_head">
            {{$t('forSalesman.titleBank.title')}}
        </div>
        <div class="right_body forms_form">
            <div class="line_pic">
                <div class="select">
                    <el-date-picker v-model="sel_day" type="month" :picker-options="pickerOptions" @change="change_date" format="yyyy-MM">
                    </el-date-picker>
                </div>
                <div class="line_body" id="form_pic"></div>
                <h3 class="note">{{$t('forSalesman.bodyBank.desDate')}}：<span class="orange">{{note_data.date}}</span> ，{{$t('forSalesman.bodyBank.desChannel')}}：<span class="orange">{{note_data.mchnt_nums}}</span>{{$t('forSalesman.bodyBank.unitPerson')}}，{{$t('forSalesman.bodyBank.desChannelNum')}}：<span class="orange">{{note_data.slsm_cnt}}</span>{{$t('forSalesman.bodyBank.unitGe')}}。</h3>
            </div>
        </div>
    ...
    methods: {
        ...
        formpic_line: function() {
            ...
            _this.from_now.setOption({
                ...
                xAxis: {
                    name: _this.$t('forSalesman.bodyBank.lineX'),
                    data: _this.form_x_data.slice(0, _this.data_len),
                },
                yAxis: {
                    name: _this.$t('forSalesman.bodyBank.lineY'),
                    type: 'value',
                    boundaryGap: ['0%', '5%'],
                },
                series: [{
