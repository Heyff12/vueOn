...
Vue.component('load', {
	props: ['visible'],
    template: '<div id="load_small_bg" v-show="visible"><div id="load_small"><i></i><br/><span>{{$t("app.dataLoading")}}</span></div></div>',
    data: function() {
        return {}
    }
})
        ...
        myvisible(val) {
            if(val==true){
                setTimeout(() => {
                  ...
                  this.$emit("on-visible-change", false); //③组件内对myvisible变更后向外部发送事件通知
                }, 3000);
            }else{
                this.$emit("on-visible-change", false); //③组件内对myvisible变更后向外部发送事件通知
            }            
